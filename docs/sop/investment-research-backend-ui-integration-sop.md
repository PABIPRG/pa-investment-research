...
- 现有投研后端增加页面读取、写入或模型工具能力；
- 新增独立投研 Python 后端及其生命周期、Profile 和打包接入；
- 为上述能力新增或更新投研专用页面、导航和智能助手联动。

不适用范围：

- 只修改后端内部实现且 HTTP 接口、operation 和 UI 行为均不变；
- 通用会话、工作区、文件、审批等能力，应优先复用现有扩展点；
- 仅用于展示的静态交互稿。
...
1. 根据第 3 节判断接入类型。
2. 使用第 4.1 节的模板记录接口、operation 和 UI 状态。
3. 现有后端能力执行第 5 节；新增独立后端还要执行第 6 节。
4. 提交 PR 前完成第 7、8、9、11 节中适用的检查。

如果只是给现有后端增加一个页面读取接口，优先走 A 类路径。只有确实需要独立进程、依赖和生命周期时，才新增 Python 后端。
...
| 阶段 | 主要工作 | 阶段产物 |
| --- | --- | --- |
| 1. 定义接口 | 写清 method、path、字段、错误和数据语义 | API 文档或 Issue 说明 |
| 2. 验证后端 | 提供身份健康检查、稳定 JSON 和可重复测试 | 真实响应和后端测试 |
| 3. 增加 Host operation | 固定 backend、method 和 path，校验 input 并管理 lease | operation、broker 映射和 contract 测试 |
| 4. 开发 UI | 建立 TypeScript view model，接入真实数据并覆盖完整交互状态 | 页面、样式和 UI 测试 |
| 5. 提交 PR | 完成适用的安全、类型、lint、构建和 Profile 检查 | 文档、测试结果和 PR |

新增独立后端时，在上述流程之外补充 Runtime、业务插件、bundle、Profile、设置页和 sidecar 接入。

如果页面提供“在智能助手中分析”，页面只通过 `prepareAssistant()` 预填正式会话输入框，由用户确认后发送，不能绕过现有模型选择、附件、审批和发送策略。
...
可以复制以下模板到 Issue 或 PR 描述；与当前接入无关的项目直接删除。
...
- 类型：A 现有后端接口 / B 模型工具 / C 独立后端 / D 产品实体能力
- backend ID：
- 是否需要模型工具：是 / 否
- 是否包含写操作：是 / 否
- 新增独立服务的理由（仅 C 类）：
...
- operation 名称：`<backend-id>.<business-action>`
- input 白名单：
- 固定 backend / method / path：
- credential 和敏感数据处理：
- 非 2xx 错误映射与可重试性：
...
- loading / refreshing：
- empty：
- partial error / page error / retry：
- stale data 与更新时间：
- 筛选、排序、分页、选择和刷新：
- 写操作 pending / success / failure / confirm：
- “在智能助手中分析”的预填提示词：
- 760px / 620px 窄屏和键盘操作：
...
operation 一旦发布就作为兼容性约定保留。HTTP path 可以在 Host 内迁移，不需要因此修改 operation 名称。
...
3. 在 Issue 或 PR 描述中列出 operation、页面入口、受影响 package 和验证项。
...
- 接口字段或单个页面变化时更新对应 API 文档和 README；通用流程或扩展点变化时更新本 SOP。
- 文档中的路径、命令和参考实现应与主分支保持一致。
- 无法完成的检查或已知限制直接写在 PR 中。
