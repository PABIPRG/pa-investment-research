...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all. [`apps/electron`](../apps/electron/README.md) reuses the Web profile's Host and client-plugin composition while an application overlay replaces its HTTP/WebSocket carrier and static server with local files plus sandboxed preload IPC. `dsh electron` selects that application process; `--profile` continues to name only a composition.
