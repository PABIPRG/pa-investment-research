...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加浏览器应用；[`dsh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器。[`apps/electron`](../apps/electron/README.md) 复用 Web profile 的 Host 与客户端插件组合，再由应用 overlay 以本地文件和沙箱化 preload IPC 取代 HTTP/WebSocket 载体及静态服务器。`dsh electron` 选择该应用进程；`--profile` 仍然只用于命名组合。
