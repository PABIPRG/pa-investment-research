...
//   import index.ts → apply(ctx, config) → ctx.tools.register ×4
// 用 mock ctx 收集注册的工具定义，断言 4 个工具名与必填字段齐全。
...
plugin.apply(fakeCtx as never, {
  adapterBaseUrl: 'http://127.0.0.1:8200',
})

const expected = ['chain_search', 'chain_profile', 'chain_graph', 'chain_expand']
...
for (const want of ['chain_search', 'chain_profile', 'chain_graph', 'chain_expand']) {
...
const LIVE_CHECK: Array<{ name: string; args: Record<string, unknown> }> = [
  { name: 'chain_search', args: { keyword: '上海家化' } },
  { name: 'chain_profile', args: { code: '600315' } },
  { name: 'chain_graph', args: { code: '600315' } },
  { name: 'chain_expand', args: { code: '600315', depth_up: 1, depth_down: 2, top_up: 3, top_down: 2 } },
]
