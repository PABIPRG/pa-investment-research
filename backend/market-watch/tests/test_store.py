...
from market_watch.store import JsonStore, JsonStoreCorruptionError
...
class JsonStoreTests(unittest.TestCase):
    ...
    def test_concurrent_same_key_mutations_preserve_every_append(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertCountEqual(stores[0].get("events", "seen_ids"), range(8))
    ...
    def test_corrupt_json_fails_without_replacing_the_original(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(JsonStoreCorruptionError, "持久化文件损坏"):
                JsonStore(root).get("watchlist", "default")

            self.assertEqual(path.read_bytes(), original)
            self.assertEqual(list(root.glob(".watchlist.json.*.tmp")), [])

    def test_replace_failure_removes_unique_temporary_file(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            store = JsonStore(root)
            store.set("watchlist", "default", [{"code": "600519"}])
            original = (root / "watchlist.json").read_bytes()

            with patch("market_watch.store.os.replace", side_effect=OSError("replace failed")):
                with self.assertRaisesRegex(OSError, "replace failed"):
                    store.set("watchlist", "default", [])

            self.assertEqual((root / "watchlist.json").read_bytes(), original)
            self.assertEqual(list(root.glob(".watchlist.json.*.tmp")), [])

    def test_empty_list_replaces_existing_state(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(store.get("watchlist", "default"), [])
