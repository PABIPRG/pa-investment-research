#!/usr/bin/env bash
# ============================================================
#  dsh-trading-core 一键启动（macOS / Linux）
#   服务1: 适配器 FastAPI  :8000（默认 engine 模式）
#   服务2: dsh Web UI      :3080（带 9 工具插件）
#   - 已运行的服务会自动跳过，不会重复启动
#   - 加参数 fake 则适配器以假任务模式启动（联调用）
# 用法: ./start.sh [fake]
# ============================================================
set -euo pipefail
cd "$(dirname "$0")"
...
RUNNER="${1:-engine}"
if [ "$RUNNER" != "fake" ] && [ "$RUNNER" != "engine" ]; then
    echo "[错误] 参数仅支持 fake / engine，收到: $RUNNER" >&2
    exit 1
fi

echo "================================================================"
echo "  dsh-trading-core 启动"
echo "================================================================"
echo
...
if curl -sf -o /dev/null http://127.0.0.1:3080 2>/dev/null; then
    echo "[注意] :3080 已有服务（可能是旧项目占用），未启动新的 dsh"
    echo "       如需本项目的 dsh，请先运行 ./stop_all.sh 清理再启动"
else
    echo "[2/2] 启动 dsh Web UI (:3080, 插件已加载)..."
    ( cd "$DSH_RUN" && nohup npx @deepseek-ai/dsh web --patch "$PATCH" >> "$LOGS/dsh.log" 2>&1 & )
    for _ in $(seq 1 120); do
        curl -sf -o /dev/null http://127.0.0.1:3080 2>/dev/null && break
        sleep 1
    done
    if ! curl -sf -o /dev/null http://127.0.0.1:3080 2>/dev/null; then
