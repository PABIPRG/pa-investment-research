...
This reference defines the shipped application selectors, profile boot, plugin management, and config-dump command modes. Argv is parsed once through [`src/args.ts`](../src/args.ts), and [`src/bin.ts`](../src/bin.ts) dynamically imports only the selected runner.
...
## Shipped application selectors

`dsh cli <task>` is an alias for `dsh --profile headless <task>`. It accepts the same launcher `--patch` and config-dump flags, then hands the remaining task text to the headless app.
...
The `investment-research` Web and Electron surfaces are mutually exclusive owners of one local product lease. An interactive Web launch names an existing surface and asks whether to replace it; Electron presents the equivalent native confirmation. Replacement is cooperative: the contender authenticates to the old owner's loopback control channel, waits for the old profile and its managed Python processes to stop, then launches. Non-interactive launches never replace a live owner implicitly and instead exit with an actionable conflict message.
...
From the repository root, run `pnpm run build` separately after a fresh checkout and whenever artifacts need updating, then use `pnpm dsh <args...>`. The `package.json` script launches `apps/cli/src/bin.ts` with `node --import tsx/esm` without building and forwards every argument. Missing Typert host artifacts fail profile boot through module-resolution errors without a build instruction. Once those host artifacts exist, missing frontend or client-plugin bundles fail at startup with an instruction to run `pnpm run build`; the Electron selector performs the same check explicitly for its three application artifacts. The launcher does not check freshness, so existing stale bundles can run older browser code until rebuilt. The process inherits the launch environment; set `NODE_USE_ENV_PROXY=1` when a supporting Node version must honor `HTTP_PROXY` and `HTTPS_PROXY`. The installed CLI launches the built `apps/cli/lib/bin.js` without rebuilding; packaged desktop users launch the installed Electron application rather than relying on the repository-sibling source selector.
