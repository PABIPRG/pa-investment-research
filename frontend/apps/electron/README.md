...
`package:electron` builds a self-contained production deployment plus the current native Python sidecar and creates an unpacked application under `apps/electron/out/`. The sidecar is copied to `Resources/investment-python`; build caches and staging directories are never copied. The final bundle is smoke-tested against its closed runtime descriptor and the sidecar is sealed read-only so a diagnostic launch cannot add Python cache files and invalidate the next startup. `make:electron` also runs Electron Forge's configured maker to create a ZIP for the current platform and architecture under `apps/electron/out/make/`. Run `node scripts/smoke-investment-python-sidecar.ts --root <Resources/investment-python>` against the final resource directory.

The sidecar builder verifies the locked Python archive SHA-256 before caching or extracting it. Cold downloads allow five attempts for transient network failures with a 30-second connection timeout, bounded backoff, and an eight-minute overall deadline; permanent HTTP failures and hash mismatches fail immediately. CI caches the Python archive by the runtime lock independently from pip and Electron downloads, so an unrelated frontend dependency update does not invalidate the Python archive cache.

macOS local builds fall back to ad-hoc signing. Set `DSH_MAC_CODESIGN_IDENTITY` (or the standard `CSC_NAME`) to a stable code-signing identity for distributable builds; the same identity is applied to Electron helpers, Python Mach-O files, and the final application. A stable identity is required for Accessibility permission to survive application updates—an ad-hoc signature's designated requirement changes whenever the bundle changes. Signing and notarization remain release gates, while the `Investment packaged sidecar` workflow performs native arm64/x64/Windows artifact smoke and ad-hoc macOS signature verification when no release identity is configured.
