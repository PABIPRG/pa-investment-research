...
macOS 持仓读取只能由用户主动点击发起。主进程提供按次授权或长期授权；长期授权只免除后续弹窗，不开启后台或定时读取，并可在设置中关闭。读取支持协作式取消并恢复应用焦点；Renderer 只拿到不透明预览会话，提交还需第二次原生确认。原生动作仅接受固定目的地，任意命令、URL 和应用路径不能由页面传入；Windows 客户端位置继续仅通过本机选择器选择 xiadan.exe。
