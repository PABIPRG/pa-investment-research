...
describe('holdings native consent', () => {
  ...
  it('cancellation never reaches native executor', async () => {
    ...
    expect(mocks.showMessageBox.mock.calls[0]![1].buttons).toEqual(['允许并继续', '取消'])
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).toContain('我们将前往')
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).toContain('交易 → 模拟 → 股票 → 持仓')
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).toContain('窗口会短暂切换到同花顺')
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).not.toContain('请手动进入')
