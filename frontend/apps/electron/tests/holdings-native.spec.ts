...
describe('holdings native consent', () => {
  ...
  it('cancellation never reaches native executor', async () => {
    ...
    expect(mocks.showMessageBox.mock.calls[0]![1].buttons).toEqual(['开始读取', '取消'])
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).toContain('请先在同花顺打开')
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).toContain('交易 → 模拟 → 股票 → 持仓')
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).toContain('仅当页面未就绪时')
    expect(mocks.showMessageBox.mock.calls[0]![1].detail).not.toContain('持仓与成交明细')
