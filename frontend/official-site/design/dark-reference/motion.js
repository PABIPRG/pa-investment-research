...
function layout(){
  ...
  $('#motion').textContent = reduced?'恢复动态':'减少动态';
...
void main(){
 ...
 p=turn(p,vec3(.16+.1*sin(uTime*.42)+uScroll*.12,-.32+.12*sin(uTime*.35),.035*sin(uTime*.28)-uScroll*.06));
...
const fragment=`precision mediump float;varying float vShade;uniform float uMode;void main(){if(length(gl_PointCoord-.5)>.5)discard;float c=uMode<.5?vShade:mix(.45,.72,vShade);gl_FragColor=vec4(vec3(c),1.);}`;
class PointScene{
 constructor(canvas,mode){
  ...
  this.uniforms=Object.fromEntries(['Time','Progress','Aspect','Size','Mode','Scroll','PointerActive','Pointer','Viewport'].map(n=>[n,gl.getUniformLocation(this.program,'u'+n)]));
 ...
 updateSpring(time){
  ...
  const progress=clamp(scrollY/this.h),ax=.16+.1*Math.sin(time*.42)+progress*.12,ay=-.32+.12*Math.sin(time*.35),az=.035*Math.sin(time*.28)-progress*.06;
  ...
  for(let i=0;i<this.count;i++){
   ...
   const depth=camera-z,px=(x*2.246/depth/(this.w/this.h)*.5+.5)*this.w,py=(.5-y*2.246/depth*.5)*this.h;
...
$('#motion').addEventListener('click',()=>{reduced=!reduced;layout();});media.addEventListener('change',e=>{reduced=e.matches;layout();});
