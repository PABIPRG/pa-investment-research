...
The Runtime is also the trust boundary for browser-initiated investment data reads. The client sends an `InvestmentDataOperation`, never a backend origin or URL. `requestInvestmentData` selects one fixed backend, HTTP method, and path, validates every input key and bound, acquires a verified backend lease, and releases that lease after the response settles. Unknown operation names, unknown input fields, unsupported enum values, and out-of-range limits fail before a backend request is dispatched.

The operation allowlist covers market observation in `market-watch`; personal research data plus analysis, brief, backtest, strategy, shadow, and evolution workflows in `trading-core`; and company search, company and entity profiles, five-column chains, bounded multi-level chains, statistics, and server-filtered network slices in `industry-chain`. Unified tasks expose only fixed start, status, and result routes. It never exposes arbitrary backend access or synthetic results.
