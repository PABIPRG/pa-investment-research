...
通知分类的归档数量按 `notification_center.tables.notifications` 的记录条数校验，不把 `schemaVersion`、`tables` 包装字段计为记录。创建与读取归档使用同一校验，空通知表可以正常备份；声明数量与实际条数不符或通知表不是列表时拒绝归档。
...
持仓同步默认返回五分钟有效的只读预览，commit 必须携带预览 token；账户、数据源或本地持仓变化会拒绝替换。Electron 会拒绝通用持仓同步入口：主进程负责产品授权、不透明预览会话、提交确认和协作式取消，后端预览 token 始终只对宿主可见。原生动作使用仅宿主可见的凭证和非 Remote 方法，只支持自管本机后台。
