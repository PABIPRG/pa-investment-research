...
在 Web 组合中，精确 SSE 路由与 `/plugins` 组合包前缀共用 Host/Origin/socket 信任栅栏和 WebAuth 授权。要求认证时，初始私有模块图必须在有效会话和受信 HTTPS 代理边界之后才能读取。组合将通道限制为最多 64 条并发流；close、error、背压和插件释放都会回收或销毁连接。其他组合保留禁用认证的兼容路径，但仍会通过共享请求信任栅栏。
