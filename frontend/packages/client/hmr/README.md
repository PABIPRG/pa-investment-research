...
In the Web composition, the exact SSE route applies the same Host/Origin/socket trust fence and WebAuth authorization as the `/plugins` bundle prefix. Required authentication therefore keeps the initial private module graph behind a valid session and trusted HTTPS proxy boundary. Each stream is bound to that session, so logout, idle or absolute expiry, and auth disposal immediately destroy it and release its slot. The composition caps the channel at 64 concurrent streams; close, error, backpressure, and plugin disposal also release or destroy connections. Other compositions retain the disabled-auth compatibility path but still pass the shared request trust fence.
