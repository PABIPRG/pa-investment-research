...
In the Web composition, the exact SSE route applies the same Host/Origin/socket trust fence and WebAuth authorization as the `/plugins` bundle prefix. Required authentication therefore keeps the initial private module graph behind a valid session and trusted HTTPS proxy boundary. The composition caps the channel at 64 concurrent streams; close, error, backpressure, and plugin disposal release or destroy connections. Other compositions retain the disabled-auth compatibility path but still pass the shared request trust fence.
