...
远程浏览器的模型页面及模型引导使用 `modelAdmin` 专用接口；本地与 Electron 沿用原接口。首次加载显示状态，权限拒绝与会话过期提供说明和重试。远程发现只支持公网 HTTPS；查询草稿地址时需明确输入测试密钥，已存密钥不会被自动发送到草稿地址。配置与密钥分阶段保存，沿用部分成功后的独立重试。
