...
Workspace-first 行为是共享默认值，而非所有产品 Profile 的必选项。Profile 可以把 `body.dataset.workspaceContextVisibility` 设为 `hidden`；此时壳层不会渲染 Workspace chip 或 picker slot，未归属 Workspace 的 Host-cwd 空白会话仍可直接输入，并使用普通 Hero placeholder。
