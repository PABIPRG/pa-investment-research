...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，并把构建后的 bundle 哈希写入启动图。组合中存在 Web server 时，它会获得 `/plugins` bundle route、sourcemap route 与 index manifest 注入。交付的 Web 组合会让该路由先通过共享 Host/Origin/socket 信任栅栏，并要求 WebAuth 权威存在后才读取 bundle；缺少鉴权时 fail closed，source map 始终不可用。桌面载体则可通过显式 `additionalPackages` 登记额外包，并直接读取同一模块图与 `bundleFile()`——它把一个 `/plugins/<id>/client.js[.map]` 路径解析为任何载体都必须提供的文件与内容类型；Electron 从其应用 scheme 提供同样这些 URL。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
