...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph. A Web server, when composed, receives the `/plugins` bundle route, source-map route, and index manifest injection. The shipped Web composition puts that route behind the shared Host/Origin/socket trust fence and requires the WebAuth authority before reading a bundle; missing authentication fails closed, and source maps remain unavailable. A desktop carrier can instead enroll explicit `additionalPackages` and read the same graph plus `bundleFile()`, which resolves one `/plugins/<id>/client.js[.map]` pathname to the file and content type any carrier must serve; Electron serves those same URLs from its application scheme. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
