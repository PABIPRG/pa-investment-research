...
- Research Workbench is the default landing page. Its holdings, risk, alerts, personalized-event, and strategy-match regions settle independently; it distinguishes current market value from cost and shows a current profit/loss card. The card opens a lazy-loaded performance dialog with since-inception, preset, and custom ranges, TWR, estimated XIRR, cost return, an interactive valuation curve with labeled axes, point labels, hover details, and keyboard point navigation, security contributions, and explicit data-quality limits. Personalized events request `comment: false`, so browsing does not depend on an LLM credential.
