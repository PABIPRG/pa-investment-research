...
- Strategy Research turns real events into hypotheses and out-of-sample backtests. Each strategy card keeps Strategy, Backtest, AI Review, Shadow Validation, and Archive on one action row; Strategy opens temporary inspection without selecting the card as the downstream work target, while Backtest opens independent task management. AI Review and Shadow Validation carry the strategy selection explicitly. Shadow Validation reads paper-account state, positions, and equity. Evolution shows attribution and a read-only plan before a second, explicit confirmation can write to the strategy store.
