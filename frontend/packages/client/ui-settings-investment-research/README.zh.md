...
`investment-research` Profile 的投研设置页面。页面顶部的**数据与备份**区块管理可迁移的 `.pabackup` 归档，支持按投研数据域选择内容、预览增量导入、逐域配置冲突规则，并可在导入前创建安全备份。Host 确认数据应用成功后，页面会自动刷新，使所有已挂载的产品界面从权威来源重新读取导入数据；导入失败时保留确认弹窗与用户选择，并在弹窗内显示错误，便于直接重试。**导出当前对话**仍是独立操作，它调用现有 `ctx.sessionLogDownload.download(currentSessionId)` 控制器，产物只包含所选对话树及其引用附件。

创建失败也在确认弹窗内显示错误，并保留所选分类供重试。Host 确认创建成功后关闭弹窗；若随后列表刷新失败，页面显示已创建的文件名，并引导用户刷新列表，无需重复创建归档。
