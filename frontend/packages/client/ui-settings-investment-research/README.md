...
Investment research Settings page for the `investment-research` Profile. Its first section, **Data & backup**, manages portable `.pabackup` archives for selected investment domains, including previewed incremental import with per-domain conflict rules and an optional pre-import safety backup. A successful import reloads the page only after the Host acknowledges the applied state, so every mounted product surface reads the imported data from its authoritative source; an import failure keeps the confirmation dialog and user choices available, with its error inside the dialog so the user can retry. **Export current conversation** remains a separate action that calls the existing `ctx.sessionLogDownload.download(currentSessionId)` controller and produces only the selected conversation tree and referenced attachments.

Create failures also appear inside the confirmation dialog and preserve the selected categories for retry. Once the Host confirms creation, the dialog closes; if refreshing the list then fails, the page reports the created filename and directs the user to refresh instead of creating another archive.

The localized `settings.section` entry also projects only the secret-free snapshot supplied by `ctx.investmentResearchRuntimeClient`, including backend ownership, health, declared tool count, credential state, and capability level. Local deployments also show the Host-provided Runtime log hint; Cloud Web readiness omits that path, and the page omits the complete log row. Internal persistence implementation details are deliberately absent from the user interface.
