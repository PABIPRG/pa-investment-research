...
- The Connection adapter maps unknown dispatch failures and business exceptions to a generic RPC `internal` response with empty details. `TypertLookupFailure` and `TypertRemoteFailure` payloads are returned unchanged because their owners must make them caller-safe before throwing; structured `TypertGatewayError` categories remain available only to same-process callers.
