...
- Connection 适配器将未知分发故障和业务异常映射为通用 RPC `internal` 响应，且不附带详细信息。`TypertLookupFailure` 与 `TypertRemoteFailure` 的载荷会原样返回，因为其所有者必须在抛出前保证调用方可见内容安全；结构化的 `TypertGatewayError` 类别仅供同进程调用方使用。
