...
describe('remote model administrator facade', () => {
  it('persists only model fields and never returns credential values or unrelated settings', async () => {
    const ctx = await harness()
    ctx.settings.register(NS, AdapterConfig)
    ctx.settings.register(settingsNamespace('web-auth'), z.object({ passwordHash: z.string() }), { base: { passwordHash: 'private-hash' } })
    const pi = settingsNamespace('llm-pi-ai')
    ctx.settings.register(pi, z.object({ providers: z.dict(z.object({ apiKeyEnv: z.string(), baseURL: z.string(), models: z.array(z.object({ id: z.string() })) })).default({}) }))
    ctx.llm.registerConfigurableProviders([{ provider: 'acme', displayName: 'Acme', settingsNs: 'llm-pi-ai', settingsPath: ['providers', 'acme'] }])
    const api = createApiProxy(ctx, DEFAULTS)
      const snapshot = expectOk(await api.modelAdmin.describe(request({})))
      expect(snapshot.namespaces.map(view => view.ns)).not.toContain('web-auth')
      expect(JSON.stringify(snapshot)).not.toContain('private-hash')
      expectErr(await api.modelAdmin.describeCredentials(request({ refs: ['PATH'] })))
      expectErr(await api.modelAdmin.setCredential(request({ ref: 'PATH', value: 'evil' })))
      expectErr(await api.modelAdmin.mutate(request({ ns: 'web-auth', ops: [{ op: 'set', path: ['passwordHash'], value: 'evil' }] })))
      expectOk(await api.modelAdmin.mutate(request({ ns: 'llm-pi-ai', ops: [{ op: 'set', path: ['providers', 'acme'], value: { apiKeyEnv: 'ACME_API_KEY', baseURL: 'https://api.example.com/v1', models: [{ id: 'test-model' }] } }] })))
      expectOk(await api.modelAdmin.setCredential(request({ ref: 'ACME_API_KEY', value: 'exclusive-test-key' })))
      const state = expectOk(await api.modelAdmin.describeCredentials(request({ refs: ['ACME_API_KEY'] })))
      expect(state.credentials['ACME_API_KEY']).toEqual({ configured: true, writable: true })
      expect(JSON.stringify(await api.modelAdmin.describe(request({})))).not.toContain('exclusive-test-key')
      expectOk(await api.modelAdmin.unsetCredential(request({ ref: 'ACME_API_KEY' })))
      expectErr(await api.modelAdmin.mutate(request({ ns: 'llm-pi-ai', ops: [{ op: 'set', path: ['providers', 'acme', 'baseURL'], value: 'http://127.0.0.1' }] })))
  })
})
