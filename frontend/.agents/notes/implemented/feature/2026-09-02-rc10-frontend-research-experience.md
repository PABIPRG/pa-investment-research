...
Strategy Research keeps lifecycle education behind a keyboard-safe help dialog and removes page-level refresh and backtest-window controls. Until the backend task-history work is delivered, the existing run action retains the established two-year request rather than presenting a false history manager. Each strategy card prioritizes security names over codes, keeps complete codes and trigger/exit rules available through hover and keyboard focus, labels sample-out evidence explicitly, and uses the portfolio daily-NAV maximum drawdown rather than the trade-sequence secondary metric. Related actions share one compact group, while Shadow Validation remains a secondary action before Archive. The Evolution dashboard separates participation state, confidence tier, and mutation origin; mutation is not a countable lifecycle group.
