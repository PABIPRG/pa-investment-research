...
The sidebar Evolution entry is a global read-only dashboard. It reads only unscoped evolution status and attribution and presents the loop switch, lifecycle groups, current decisions, active-strategy lineage, and recent automatically applied actions. Strategy rows, lifecycle entries, lineage nodes, and history actions navigate to Strategy Research stage four with a controlled strategy id and optional return group.
