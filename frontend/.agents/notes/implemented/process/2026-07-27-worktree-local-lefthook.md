...
Hook installation is worktree-scoped. With `CI=true` or `GITHUB_ACTIONS=true`, the installer returns before Git discovery or mutation because automated jobs do not consume contributor hooks. A checkout without `node_modules/.bin/lefthook` returns after Git root discovery but before package resolution or repository mutation, so a production-only install can run `postinstall`; when the binary exists, package resolution and installation failures remain fatal. The standalone investment packaging entry first runs a frozen `pnpm install --prod=false` with noninteractive module-directory replacement enabled, restoring workspace development tools that a previous production deploy removed. It then runs constraints and the Electron build with `pnpm_config_verify_deps_before_run=warn`, so those commands use the explicitly restored tree instead of triggering an implicit dependency repair. Otherwise, the installer requires Git 2.26 or newer so `git config --show-scope` can report which scope supplied a value, upgrades a format-0 repository to format 1, enables `extensions.worktreeConfig`, and assigns the current worktree an absolute `core.hooksPath` at `$GIT_DIR/dsh-hooks`.
