...
共享 home-path 包从一个挂载根解析 Host 设置、profile、会话、附件、storage、备份与全部 backend 子树。Runtime 初始化会创建带版本的空目录布局。Runtime 迁移只通过同级 staging 目录复制已声明的持久清单，校验普通文件后原子发布目标，不改变来源。它排除凭据和运行期产物，保持 PAB-14 业务归档语义独立，并要求在线来源中的 SQLite 使用显式一致性备份操作。
