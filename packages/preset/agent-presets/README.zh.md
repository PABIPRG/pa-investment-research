...
行的**包名**从宿主组装解析，而非从 preset 目录解析。Loader 通常按 entry 所属树的 `baseUrl` 解析，而对 preset 而言那就是组装文件所在之处；本地创作的 preset 位于用户主目录之下，Node 向上查找 `node_modules` 永远够不到 harness，因此每一个 `@deepseek-ai/dsh-*` 行都会导入失败。挂载把裸标识符交给根 Loader 的公开 `import()`，由后者持有宿主基址以及承载方在其上安装的解析方式——没有 Node 模块加载器内部接口的承载方以自己的方式解析宿主包，preset 子树继承这一解析而不是导入失败。
