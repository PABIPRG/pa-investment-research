...
# 风险偏好约束块：注入风险经理提示词，让最终决策随用户风险偏好变化。
# balanced 为空 → 提示词与引擎现状完全等价（零回归）。
RISK_PREFERENCE_PROMPTS = {
    "conservative": (
        "【风险偏好约束 · 保守型投资者】\n"
        "您正在为风险厌恶型（保守）投资者做决策，其核心目标是保住本金、控制回撤。\n"
        "1. 风险评估从严：任何不确定性都应当提高风险评分；风险偏高时宁可建议减仓/卖出，也不建议持有或买入。\n"
        "2. 只有在证据非常充分时才给出买入建议；弱信号、信号矛盾时一律不追高。\n"
        "3. 目标价保守，留足安全边际；宁错过，不套牢。\n"
        "4. 在最终建议中附一句风险提示（当前主要风险点与应对）。\n"
    ),
    "balanced": "",
    "aggressive": (
        "【风险偏好约束 · 进取型投资者】\n"
        "您正在为风险承受型（进取）投资者做决策，其追求高收益，能够接受较大波动与回撤。\n"
        "1. 风险评估可适度放宽：不因短期波动轻易看空，重点评估上行弹性与趋势。\n"
        "2. 可以把握成长/题材机会，技术面与情绪面权重可更高；趋势明确、逻辑顺畅时可给出买入。\n"
        "3. 目标价可更有想象力，但必须给出明确的止损位。\n"
        "4. 在最终建议中明确标注止损位与风险位。\n"
    ),
}


def create_risk_manager(llm, memory, risk_preference: str = "balanced"):
    def risk_manager_node(state) -> dict:
        ...
        risk_constraint = RISK_PREFERENCE_PROMPTS.get(risk_preference, "")
