...
0.2.0 应用身份使用用户提供的蓝底白色趋势线图标（APP-ICON-001 / PAB-22）。源文件与多平台资源说明见 [`应用图标`](frontend/apps/electron/assets/README.md)。Web 与桌面端复用同一图形；业务功能图标沿用现有体系。小尺寸、系统裁切和安装升级后的显示需要真实验证，未经裁切验证的资源不声明为 maskable。
