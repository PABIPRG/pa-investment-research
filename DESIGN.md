...
一级路由必须显式使用共享的路由表面契约，不能只依赖裸滚动容器。研究工作台、智能分析、实时盯盘、策略研究和产业链是首批强制采用该契约的页面；后续新增一级路由应沿用同一结构，除非产品目标要求不同层级并在变更说明中记录原因。

- 路由画布与自进化页面保持一致，使用 `--dsw-alias-bg-layer-1`；浅色主题下保持白色背景，不用灰色画布代替模块分组。页面标题区与正文使用 `--dsw-alias-border-l2` 分隔。
- 大模块默认使用 `--dsw-alias-bg-base` 与 `--dsw-alias-border-l2`，采用 `12px` 圆角、`16px` 内边距和 `20px` 同级间距；非浮层模块不使用阴影制造层级。
- 内部静态分组默认不用完整边框，优先使用背景色阶、`12 / 16px` 留白和 `--dsw-alias-border-l1` 单向分隔线。
- 同一视觉区域禁止连续两层相同强度的完整边框。只有可独立操作、可重排、可选择或边界本身携带业务语义的内容，才保留内部卡片边界。
- 成功、警告和错误提示与相邻内容至少 `12px` 间距，同时保留文本、语义角色和非颜色状态线索。
- 新页面或大模块必须检查浅色与深色主题，并至少验证 1440px 和 1024px；支持窄屏时继续检查约 768px 与 390px。
- 偏离共享契约时使用语义清晰的页面专属类，禁止用全局选择器或字面颜色覆盖；变更说明必须记录偏离原因和真实产品验收证据。
...
0.2.0 应用身份使用用户提供的蓝底白色趋势线图标（APP-ICON-001 / PAB-22）。1024×1024 方形母版是唯一品牌源；Web 登录页、应用内品牌区与桌面端复用同一图形，业务功能图标使用稳定的 SVG 体系。源文件、多平台派生方式与验证说明见 [`应用图标`](frontend/apps/electron/assets/README.md)。

Apple 平台使用未预裁切的方形资源，由系统施加最终遮罩；Windows ICO、Web favicon 与 PWA `purpose: any` 使用确定性生成的透明圆角底板。PWA `purpose: maskable` 使用不透明满幅背景，白色趋势线完整位于 W3C 定义的中心 40% 半径安全圆内。任何新派生资源都必须由同一母版可重复生成，并以像素级透明度、安全区、容器帧和最终打包资源检查锁定；小尺寸、系统裁切和安装升级后的显示仍需在目标平台真实验证。
...
- 形状使用语义层级：紧凑控件 `6px`、普通按钮和输入框 `8px`、模块容器 `12px`、弹层 `16px`；只有状态、标签和明确胶囊形控件使用全圆角。共享实现对应 `--dsw-alias-radius-compact/control/module/overlay/pill`，页面不得自行漂移。
...
- 业务单选下拉使用共享 `Select`，不直接渲染原生 `<select>`；宽屏分段选择在窄屏容纳不下时可复用同一状态切换为 `Select`。保留未知值、加载、禁用与保存失败行为，不能为了控件统一擅自选中第一项。
- 简短术语和计算口径使用共享 `HelpPopover`，点击或触屏展开锚定说明，不再叠加模态遮罩与“知道了”确认。提交、危险确认仍使用共享 `Modal`；文件、日期等浏览器专属能力及复杂编辑器不做机械替换。
...
- 持仓明细保留为父弹窗，成交录入、成交记录、券商同步和导入使用独立子弹窗，不替换父框内容。子框关闭仅返回父持仓；底部关闭使用红色圆角长方形按钮，不再提供重复的顶部返回按钮。叠层、键盘和焦点行为交由现有 UI 库管理，只有最上层响应关闭。
- 持仓数量与成本在原表格行编辑，列宽不因编辑状态变化；保存或取消后回到本行操作。行内使用 UI 库的紧凑文字按钮，买入、卖出、历史、编辑与删除横向对齐，风险状态、说明图标与设置相邻排列。新成交的标的支持名称和代码搜索，展示名称及代码并要求选择匹配结果；修改搜索词即清除旧选择与预览。
- 成交证券搜索使用 UI 库锚定浮层，展开不改变表单高度或字段位置；结果局部滚动，Esc 先收起结果再关闭子弹窗，Tab 继续到下一字段。风险设置表单正文使用 `0.8571rem`、分组标题 `0.9286rem`、辅助提示 `0.7857rem`，不扩大共享弹窗的字号。
...
- 观察室以现有持仓账户为唯一资产与操作事实源，不展示策略列表、策略详情，不汇总影子策略的虚拟本金、持仓或收益。导入、导出、重置及持仓变化需如实留痕；数据调整不等于买卖成交或投资收益。
- 观察室使用纯色背景；金额沿用系统无衬线字体并使用齐线等宽数字；按钮统一为 `8px` 小圆角矩形，不使用胶囊按钮，也不展示“免登录 · 只读”等实现属性标签。
- 月历逐日区分交易日、休市与交易日待确认；交易日历状态与是否存在账户快照独立，缺少可信日历时不得把工作日推断为交易日。
- 公开观察室是独立部署的只读 Web 表面，默认域名为 `pair-observe.xiexin.dev`；它不复用官网部署目录，也不承载登录、交易、持仓编辑、同步或任务创建入口。
- 页面首屏优先呈现日期、总权益、累计盈亏、现金、持仓市值与数据新鲜度；所有区块必须来自同一公开账户快照与 revision，缺失日期不得回退或插值。
- 金额、估值持仓、曲线与盈亏日历共享账户快照口径；操作时间线独立按截止日期读取。缺少资金快照时金额显示“—”，不能连带隐藏已公开操作，亦不能把操作记录当作权益已可用。导入／重置详情展示数量与成本校正，时间标明“记录于”，不冒充成交时间。
- 时间切片支持逐日切换、回到当日和按留存快照播放；历史日期暂停自动刷新，并明确表现为禁用状态。
- 公开活动只展示经批准的摘要，可按类型与状态筛选；持仓和运行过程允许展开查看，但仍保持只读，并继续隐藏账户标识、凭据、原始任务参数与私有链接。
- 页面支持 1440 / 1024 / 768 / 390px、浅色与深色主题；窄屏采用重排，不允许页面级横向滚动。
